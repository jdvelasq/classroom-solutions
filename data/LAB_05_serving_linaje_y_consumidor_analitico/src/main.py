...
def main():
    request = {
        ...
        "grain": "factory_date,factory_id",
        "owner": "analytics_team",
    }
    production = pd.read_csv(ROOT / "data/machine_throughput_export.csv")
    view = production.groupby(
        ["factory_date", "factory_id"], as_index=False
    ).daily_units_produced.sum()
    ...
    manifest = {
        ...
        "schema": {
            "factory_date": "date",
            "factory_id": "integer",
            "daily_units_produced": "number",
        },
        "source_asset": "machine_throughput",
    }
    (output / "serving_manifest.json").write_text(
    ...
    lineage = pd.DataFrame(
        [
            [
                "machine_throughput",
                "factory_daily_serving_view",
                "aggregate daily production by factory",
            ]
        ],
