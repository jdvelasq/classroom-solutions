...
def test_database_preserves_the_grain_of_the_source_extract():
    ...
    with sqlite3.connect(OUTPUT) as connection:
        ...
        assert tables == {"categories", "artworks", "creators", "artwork_creators", "citations"}
        assert connection.execute("SELECT COUNT(*) FROM artworks").fetchone()[0] == raw.itemid.nunique()
        expected_citations = raw.dropna(subset=["citation"])[["itemid", "citation"]].drop_duplicates().shape[0]
        assert connection.execute("SELECT COUNT(*) FROM citations").fetchone()[0] == expected_citations
...
def test_many_to_many_creator_relationship_has_valid_references():
    with sqlite3.connect(OUTPUT) as connection:
        ...
        """).fetchone()[0]
        assert invalid == 0
        assert connection.execute("SELECT COUNT(*) FROM artwork_creators").fetchone()[0] > 0
