...
def main():
    ...
    feed = pd.concat([source, source.iloc[[0, 1]].assign(agi_stub=[7, 1], N1=[-1, source.iloc[1].N1])], ignore_index=True)
    key = feed[["zipcode", "agi_stub"]].duplicated(keep=False)
    violations = pd.DataFrame({"invalid_state": feed.STATE.ne("VT") | feed.STATEFIPS.ne(50), "invalid_income_group": ~feed.agi_stub.between(1, 6), "duplicate_postal_income_key": key, "negative_return_count": feed.N1.lt(0)})
    reasons = violations.apply(lambda row: "|".join(row.index[row].tolist()), axis=1)
    accepted, quarantined = feed.loc[reasons.eq("")].copy(), feed.loc[reasons.ne("")].copy()
    ...
    report = {"input_rows": len(feed), "accepted_rows": len(accepted), "quarantined_rows": len(quarantined), "reasons": violations.sum().astype(int).to_dict()}
