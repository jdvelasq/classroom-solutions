...
def main():
    ...
    source["operation_id"] = (
        source.factory_id.astype(str)
        + "-"
        + source.machine_id.astype(str)
        + "-"
        + source.factory_date
    )
    baseline = source.iloc[:20].assign(updated_at="2019-11-01T08:00:00")
    correction = baseline.iloc[[1]].assign(
        daily_units_produced=lambda x: x.daily_units_produced + 100,
        updated_at="2019-11-02T08:00:00",
    )
    ...
    current = (
        pd.concat([baseline, batch], ignore_index=True)
    ...
    reconciliation = {
        ...
        "total_units": int(current.daily_units_produced.sum()),
        "unique_operations": int(current.operation_id.nunique()),
    }
