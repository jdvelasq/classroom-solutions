...
def main():
    ...
    metrics = events.groupby("event_hour", as_index=False).agg(events=("event_id", "count"), delivered=("status", lambda values: int((values == "delivered").sum())), failed=("status", lambda values: int((values == "failed").sum())))
    ...
    report = {"allowed_lateness_minutes": policy["allowed_lateness_minutes"], "late_event_count": len(late), "event_count": len(events)}
