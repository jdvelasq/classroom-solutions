...
def main():
    ...
    events["event_time"] = pd.to_datetime(
        events["eventDate"].str.replace("-", " ", n=3) + " " + events["eventTime"],
    ...
    events["status"] = (
        events["eventType"].eq("Normal").map({True: "delivered", False: "failed"})
    )
    events["lateness_minutes"] = (
        events.arrival_time - events.event_time
    ).dt.total_seconds() / 60
    late = events.loc[
        events.lateness_minutes > policy["allowed_lateness_minutes"]
    ].copy()
    accepted = events.loc[
        events.lateness_minutes <= policy["allowed_lateness_minutes"]
    ].copy()
    accepted["event_hour"] = accepted.event_time.dt.floor("h")
    metrics = accepted.groupby("event_hour", as_index=False).agg(
        events=("event_id", "count"),
        delivered=("status", lambda values: int((values == "delivered").sum())),
    ...
    report = {
        "allowed_lateness_minutes": policy["allowed_lateness_minutes"],
