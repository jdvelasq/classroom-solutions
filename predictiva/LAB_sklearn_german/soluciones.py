...
def pregunta_01():
    ...
    df = pd.read_csv("german.csv")  # df = ____

    # Asigne la columna `default` a la variable `y`.
    y = df["default"]  # ____ = ____

    # Asigne una copia del dataframe `df` a la variable `X`.
    X = df.copy()  # ____ = ____.____()

    # Remueva la columna `default` del DataFrame `X`.
    X.pop("default")  # ____.____(____)
...
def pregunta_02():
    ...
    from sklearn.model_selection import train_test_split  # from ____ import ____
    ...
    (  # (
        ...
        y_train,  # y_train,
        y_test,  #  y_test,
    ) = train_test_split(  # ) = ____(
        X,  # ____,
        y,  # ____,
        test_size=100,  # test_size=____,
        random_state=123,  # random_state=____,
    )  # )
...
def pregunta_03():
    ...
    from sklearn.compose import make_column_transformer  # from ____ import ____

    # Importe Pipeline
    from sklearn.pipeline import Pipeline  # from ____ import ____

    # Importe OneHotEncoder
    from sklearn.preprocessing import OneHotEncoder  # from ____ import ____

    # Importe SVC
    from sklearn.svm import SVC  # from ____ import ____
    ...
    columnTransformer = make_column_transformer(
        (
            OneHotEncoder(),  # ____(),
            make_column_selector(dtype_include=object),  # ____(____=____),
        ),
        remainder="passthrough",  # remainder=____,
    )

    # Cree un pipeline que contenga el columnTransformer y el modelo SVC.
    pipeline = Pipeline(  # pipeline = ____(
        steps=[
            ("transformer", columnTransformer),  # ("____", ____),
            ("svc", SVC()),  # ("____", ____),
        ],
    )

    # Entrene el pipeline con los datos de entrenamiento.
    pipeline.fit(X_train, y_train)  # ____.____(____, ____)
...
def pregunta_04():
    ...
    from sklearn.metrics import confusion_matrix  # from ____ import ____
    ...
    cfm_train = confusion_matrix(  # cfm_train = ____(
        y_true=y_train,  # y_true=____,
        y_pred=pipeline.predict(X_train),  # y_pred=____.____(____),
    )

    cfm_test = confusion_matrix(  # cfm_test = ____(
        y_true=y_test,  # y_true=____,
        y_pred=pipeline.predict(X_test),  # y_pred=____.____(____),
    )
